...
| P5 | 營運總覽 KPI（8 指標定稿、Admin vs Shop） | `views/ops-overview.js` + `src/routes/dashboard-ops.routes.js`（`GET /api/dashboard/ops-snapshot`）+ `#overview`；素材／商店數等見回應 `meta` |
...
- [x] P5 玩法入口「載入範圍快照」條（`quest-chains.js`）
- [x] P5 營運總覽頁 v2：8 項定稿 KPI + 聚合 API、`#overview`、Admin／Shop／Staff 範圍說明（`ops-overview.js` + `dashboard-ops.routes.js` + `role-context.js`）
...
最後更新：2026-05-02（營運總覽 v2 聚合 API、快照條、詳情 scope、素材標籤、治理頁 scope、列表工具列）

## 正規化對齊聲明

- `public/js/staff-dashboard.js` 僅增加 `loadOpsOverview` 傳入 `lazyLoad`（與 `loadBillingDashboard` 相同模式），不內嵌業務邏輯。
